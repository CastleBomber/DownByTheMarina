...
'''
    freq
        ndx used with height dictionary
        "might be amplitude*" - Harms
    bang
        == height[freq]
...
'''
def RGBLines(self, R, G, B, freq, bang):
    offset_canvas = self

    if(freq == 3 and bang >= 2):
        # Area2
    ...
    if(freq == 5 and bang >= 1 or (freq == 4 and bang >= 2)):
        # Area4
...
def SinglePixel(self, R, G, B):
    #offset_canvas = self.matrix.CreateFrameCanvas()
    offset_canvas = self
    offset_canvas.SetPixel( 1, 11, R, G, B)
    #offset_canvas = self.matrix.SwapOnVSync(offset_canvas)
